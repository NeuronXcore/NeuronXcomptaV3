...
| **Vérification plaquette comptable** | **Workflow itératif** de validation/challenge de la plaquette annuelle reçue du comptable (déclaration 2035) AVANT édition de la déclaration d'impôt. Comparaison ligne à ligne des 27 postes PCG avec les agrégats NeuronX, statuts par item (`non_revu`/`ok`/`a_challenger`/`refus_justifie`/`en_discussion`/`resolu`), commentaires inline, journal d'échanges avec le comptable. **Mapping PCG → catégories NeuronX** dans `data/plaquette_pcg_mapping.json` (versionné, extensible multi-cabinets). Flags spéciaux par compte : `is_bilan`, `is_dotation`, `is_recettes`, `split_csg_deductible`, `split_urssaf_cotisations`, `apply_quote_part_vehicule` (multiplie par le ratio du barème véhicule). **Stockage** `data/plaquette_check/{year}.json` (un fichier/exercice). **PDF rapport** : ReportLab A4 portrait avec 6 sections (logo + synthèse BNC 3-col Plaquette/NeuronX/Écart/vs-N-1 + callout pédagogique + récap statuts + anomalies à régulariser avec **top 10 ops drill-down** par item + points méthodologiques + tableau complet 28 lignes + **annexe juridique** détection auto via regex sur commentaires : 8 références BOI/CGI avec titre + extrait + URL BOFIP/Légifrance). **Auto-replace** : `_delete_previous_reports(year)` avant chaque génération → **1 seul rapport actif par exercice** dans la GED. **Bouton drawer GED** "Ouvrir vérification (N à challenger)" sur les docs `plaquette_comptable` → `PlaquetteCheckDrawer` (1100px) avec **5 onglets** : Comparatif (header sticky, drill-down ops, bandeau synthèse BNC), Saisie manuelle, Email challenge (envoi groupé Rapport + Plaquette en 1 clic), Archives (historique PDF avec actions Eye/Download/Trash), Journal (timeline + modal "Loguer réponse comptable" avec batch update statuts). Endpoints `/api/plaquette/*` (13 routes). Script `scripts/seed_plaquette_2025.py` pour pré-remplir un exercice. Type GED `plaquette_comptable`. **Session 39 P1** : cycle de vie 3 statuts `en_cours → validation_finale → declare` (irréversible) avec snapshot atomique PDF watermarké + protection GED + attachements journal (PDF/PNG/JPG/WEBP/EML/ZIP ≤ 10 Mo). **Session 39 P2** : évaluation `risque_fiscal` par item (`faible`/`modere`/`eleve`/`critique`) calculée automatiquement par règles BNC SCP (catégories sensibles, forfaits, taux justifs, écarts N-1), surchargeable manuellement. Section 7 « Préparation contrôle fiscal » dans le PDF avec top 5 risques + annexe juridique enrichie (12 refs BOI/CGI dont prescription L169 LPF, conservation L102 B, DAS-2). **Session 39 P3** : **entrée sidebar** « Vérification plaquette » dans groupe CLÔTURE (bouton-drawer, après Check d'envoi, icône `FileSearch`) avec **badge dynamique** consommant un endpoint léger `GET /{year}/summary` (Cache-Control 10s, pas de recalcul NeuronX) — priorités vert (déclaré) / rouge (risque critique) / orange (risque élevé) / ambre (items à challenger). Card info ambre « Aucune plaquette téléversée » dans l'onglet Comparatif quand `!ged_doc_id` (permet la pré-réception). Aujourd'hui 19 routes |
...
| **Livret comptable** | **Synthèse narrative annuelle vivante** (route `/livret`, sidebar **ANALYSE**, icône `BookOpen`). Agrégateur sur les services métier existants — n'introduit aucune donnée nouvelle. **9 chapitres** (Synthèse · Recettes · Charges pro · Forfaitaires · Cotisations sociales · Amortissements · Provisions · BNC fiscal · Annexes) consommés par les 3 vues via une architecture commune. **Refetch live** TanStack 60s + `refetchOnWindowFocus` + invalidation immédiate sur les mutations clés (édition op, ventilation, lock, lettrage, association justif, dotation amort, forfait, liasse). **Comparaison N-1** activable via toggle 3 états (`Sans / YTD comparable / Année pleine`) — annote `delta_n1` sur metrics, totaux chapitre, totaux sous-cat (matching par nom + lignes fantômes orphelines), et série `solde_n1` overlay sur la cadence mensuelle. Pastilles `LivretDeltaPill` colorées vert/rouge selon favorabilité métier (recettes/BNC/provisions = up favorable, charges = down favorable). Cache mémoire TTL 60s pour rester sous 1.5s en mode comparaison. **Snapshots archivables** : 3 types (`auto_monthly` job mensuel asyncio coopératif tick 1h fenêtre 1er-mois 02h-04h ; `cloture` hook `check_envoi_service.validate_instance` quand `period == ANNUAL` ; `manual` POST utilisateur). Génère 2 fichiers — HTML autonome (CSS+JS inline, aucun fetch, consultable hors-ligne file:// USB email) et PDF paginé ReportLab (~36 pages avec graphiques). Stockage `data/livret_snapshots/{year}/` + `manifest.json` (lock fichier `fcntl.flock` + sauvegarde `.bak`). Page `/livret/:year/archives` avec 4 actions par row (Eye HTML in-app via iframe sandbox / FileText PDF via `PdfPreviewDrawer` / Download menu blob HTML+PDF / Trash2 avec garde stricte cloture `force=true`). **Graphiques** (Phase 5) via architecture commune `ChartConfig` Pydantic — 3 charts MVP : **donut top-8 + Autres** chap 03 (drill-down Recharts au clic vers `CategoryDetailDrawer` existant), **waterfall BNC** chap 08 (Recettes + Charges − Dotations − Forfaits = BNC, barres cumulatives stacked invisible+visible), **cadence mensuelle** chap 01 (Recharts `<Brush>` pour zoom plage de mois). 3 renderers consomment le même hex de couleur : SVG inline pur (HTML autonome, vanilla JS tooltip) · matplotlib Agg PNG embed `RLImage` (PDF, lazy import) · Recharts ResponsiveContainer (React live). Endpoints `/api/livret/{year}` (avec `?as_of=` + `?compare_n1=ytd_comparable\|annee_pleine`) + `/api/livret/{year}/{metadata,projection}` + 6 endpoints snapshots `/api/livret/snapshots/*` |
...
| **Amortissements** | Registre immobilisations, calcul dotations linéaire **strict BNC** (dégressif legacy en lecture seule), détection auto candidates (> 500€, strict `Catégorie == "Matériel"`), plafonds véhicules CO2, cessions avec plus/moins-value, moteur calcul temps réel, **reprise d'exercice antérieur** (3 champs `exercice_entree_neuronx` + `amortissements_anterieurs` + `vnc_ouverture` avec auto-suggestion `compute-backfill`), **OD dotation au 31/12** générable depuis le module : PDF rapport multi-format (PDF/CSV/XLSX) ReportLab paysage A4 avec **colonne `Origine`** (NeuronX / Reprise badge ambre) **+ colonne `Justificatif`** (✓ filename violet ou ✗ gris, ligne TOTAL `N/M` ambre/vert), tableau registre, totaux violet, art. 39-1-2° CGI + entrée GED + intégration export ZIP (3 fichiers `Amortissements/` strictement identiques à ceux servis via `/reports`), idempotent, task auto `dotation_manquante` HIGH si exercice clos sans OD. **Anti-duplication B3** : OD/ZIP/UI Rapports consomment 2 templates unifiés (`amortissements_registre`, `amortissements_dotations`) via `report_service.get_or_generate` — 1 PDF, 1 entrée GED, 1 fichier disque par couple `(filters, format)`. Suppression de l'OD préserve le PDF du rapport (vit sa vie en GED). **Annotation inline (Session 36)** : `designation` éditable au double-clic ou crayon hover (Enter commit / Esc cancel), italique gris si libellé bancaire brut (heuristique `isLibelleBrut` sur préfixes PRLV/CB /VIR/SEPA/RETRAIT/CHQ/PAIEMENT). **Colonne paperclip** dans le registre : clic vert → `PreviewSubDrawer` 700px standalone (backdrop noir/blur) → bouton « Voir en plein écran » → `JustifPreviewLightbox` 90vw × 90vh (composant partagé extrait de `PendingScansWidget`), paperclip muted barré + toast info si pas de justif. **Drawer édition** : section « Opération source & justificatif » (parité avec mode candidate) via `useImmobilisationSource(immo.id)` + helper Lightbulb 💡 sous Désignation. **Bouton « Envoyer au comptable » (Session 35)** dans le header — auto-génère les 2 rapports manquants pour l'année + pré-coche dans le drawer global avec tous les justifs liés ; le ZIP final inclut un sous-dossier dédié `Justificatifs_immobilisations/` dédupliqué (priorité au sous-dossier dédié si un justif est aussi coché manuellement). `linked_justifs` stocké dans `rapport_meta` GED, gelé à la génération |
...
| `GET` | `/api/plaquette/templates` | Lister les cabinets comptables disponibles dans le mapping PCG |
| `GET` | `/api/plaquette/mapping` | Mapping PCG complet (lecture seule) |
| `GET` | `/api/plaquette/{year}` | Charger/créer le `PlaquetteCheck` de l'année (recalcule `montant_neuronx` à la volée) |
| `GET` | `/api/plaquette/{year}/exists` | Boolean : existe sans création |
| `GET` | `/api/plaquette/{year}/summary` | **Session 39 P3** — Résumé léger pour badge sidebar (compteurs par statut + risque, sans recalcul NeuronX, Cache-Control 10s, 200 `{exists:false}` si absent) |
| `POST` | `/api/plaquette/{year}/items` | Ajout d'un item manuel |
| `PATCH` | `/api/plaquette/{year}/items/{item_id}` | Édition montant/statut/commentaire |
| `DELETE` | `/api/plaquette/{year}/items/{item_id}` | Suppression d'un item |
| `GET` | `/api/plaquette/{year}/items/{item_id}/ops` | Drill-down ops NeuronX (top 100 triées par débit) |
| `PATCH` | `/api/plaquette/{year}/totaux` | Édition totaux 2035 (recettes/dépenses/bénéfice + N-1) |
| `POST` | `/api/plaquette/{year}/set-ged-ref` | Lier le `PlaquetteCheck` à un doc GED existant |
| `POST` | `/api/plaquette/{year}/generate-challenge-email` | Génère subject + body texte agrégeant les items `a_challenger` |
| `POST` | `/api/plaquette/{year}/generate-pdf-report` | PDF A4 + register GED + **auto-replace** anciens (retour `{filename, ged_doc_id, size_bytes, replaced_count}`) |
| `POST` | `/api/plaquette/{year}/prepare-email-bundle` | Orchestre génération PDF + email + retourne attachments `[rapport, plaquette]` |
| `POST` | `/api/plaquette/{year}/log-comptable-response` | Crée `JournalEntry` `email_in` + bascule N items en batch (préfixe `[YYYY-MM-DD réponse comptable]` au commentaire) |
| `POST` | `/api/plaquette/{year}/journal` | Ajoute une entrée journal manuelle |
| `GET` | `/api/plaquette/{year}/reports` | Historique des rapports archivés (filtre GED `source_module=plaquette`) |
| `DELETE` | `/api/plaquette/{year}/reports/{filename}` | Suppression d'un rapport archivé (GED + disque) |
| `PATCH` | `/api/plaquette/{year}/status` | **P1** — Transition statut (`en_cours ↔ validation_finale`, refuse `→declare`) |
| `POST` | `/api/plaquette/{year}/finalize` | **P1** — Transition atomique `→declare` (PDF watermarké + snapshot + GED `protected=true`) |
| `POST` | `/api/plaquette/{year}/journal/{entry_id}/attachments` | **P1** — Upload attachement journal (multipart, ≤10Mo, PDF/PNG/JPG/WEBP/EML/ZIP) |
| `GET` | `/api/plaquette/{year}/journal/{entry_id}/attachments/{filename}` | **P1** — Preview/download inline d'un attachement |
| `DELETE` | `/api/plaquette/{year}/journal/{entry_id}/attachments/{filename}` | **P1** — Suppression d'un attachement |
| `GET` | `/api/plaquette/{year}/journal/grouped-by-item` | **P1** — Vue alternative journal groupée par item |
| `POST` | `/api/plaquette/{year}/risque/recompute` | **P2** — Force recalcul risque tous items (423 si DECLARE) |
| `PATCH` | `/api/plaquette/{year}/items/{item_id}/risque` | **P2** — Override manuel niveau risque (motif obligatoire, 423 si DECLARE) |
| `DELETE` | `/api/plaquette/{year}/items/{item_id}/risque/override` | **P2** — Reset risque en mode auto + recompute immédiat |
| `GET` | `/api/plaquette/{year}/risque/top?limit=5` | **P2** — Top N risques (autorisé en DECLARE, lecture seule) |
