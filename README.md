...
| **Pipeline** | **Pipeline comptable interactif** : stepper 6 étapes, progression globale, sélecteur mois/année, **widget « Scans en attente d'association »** (filtré par year+month, sections OCR récents / Fac-similés, lien « Traiter » vers Justificatifs), **badge orange sidebar** sur `/ocr` (compteur pending scans), **titre d'onglet navigateur** synchronisé avec la route courante (utile en multi-tabs) |
...
| `POST` | `/api/rapprochement/associate-manual` | Association manuelle (supporte `ventilation_index` pour cibler une sous-ligne) |
