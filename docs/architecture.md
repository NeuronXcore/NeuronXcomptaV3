...
**Session 39 P1 — Cycle de vie + finalisation atomique** : machine d'état 3 statuts `en_cours → validation_finale → declare` (irréversible). Transitions `en_cours ↔ validation_finale` libres (PATCH `/status`), `→declare` UNIQUEMENT via `POST /finalize` qui chaîne en atomique : (1) vérifie `status==validation_finale`, (2) génère PDF watermarké via `plaquette_report_service.generate_and_register(year, final=True)` (overlay « VERSION DÉFINITIVE » 48pt rotate 45° alpha 0.18 + sous-titre « Déclarée le JJ/MM/AAAA »), (3) copie `{year}.json → {year}/final_snapshot.json` (atomique via tempfile + `os.replace`), (4) register en GED avec `protected=True` (`ged_service.delete_document` retourne False si protected), (5) update `status=declare` + `declared_at` + `declaration_ref`. Garde modifiabilité `_guard_modifiable(data)` lève `PermissionError` (→ HTTP 423) sur les mutations items/totaux/set-ged-ref si `!is_item_modifiable(check)`. Journal toujours appendable (questions fiscalistes post-déclaration). Attachements journal (PDF/PNG/JPG/WEBP/EML/ZIP ≤ 10 Mo) stockés `data/plaquette_check/{year}/journal_attachments/{slug}_{token8}.{ext}` (anti-collision via `secrets.token_hex(4)`).

**Session 39 P2 — Évaluation risque fiscal** : module pur `plaquette_risque_service.py` (~400 LOC) qui note chaque item de la 2035 selon `niveau ∈ {faible, modere, eleve, critique}` calculé par règles métier BNC SCP (drivers aggravants `+1` : categorie_sensible, forfait_applique, taux_justif_bas, ecart_n1_anormal, montant_eleve_sensible ; atténuants `-1` : boi_cgi_cite, statut_resolu, statut_refus_justifie). Seuils `SCORE_THRESHOLDS = (0, 2, 3, 999)` — `critique` réservé aux overrides manuels. Score global pondéré `0..3.0` (moyenne par `montant_neuronx`). Cache implicite via `last_evaluated_at >= last_modified_at` (skip silencieux), recalcule à chaque GET sauf si `status==declare`. Override manuel via `PATCH /items/{id}/risque` avec motif obligatoire — figer `auto_calcule=False` + `overridden_niveau`. PDF gagne une **section 7 « Préparation contrôle fiscal »** insérée avant l'annexe juridique (renumérotée 8), avec top 5 risques triés et 4 nouvelles refs `_BOI_CGI_REFERENCES` auto-injectées (`art_L169_LPF` prescription 3 ans, `art_L102_B_LPF` conservation 6 ans, `art_240_CGI_DAS2`, `art_39_4_CGI` plafonds véhicule).

**Session 39 P3 — Entrée sidebar + badge dynamique** : le drawer n'est plus accessible uniquement depuis la GED — une entrée bouton-drawer dans le groupe **CLÔTURE** de la sidebar (après `Check d'envoi`, icône `FileSearch`) ouvre désormais le drawer directement sur `selectedYear` (`useFiscalYearStore`). Bénéfice : **anticiper** la vérification (consultation des agrégats NeuronX + saisie manuelle) **avant** réception de la plaquette du comptable.

Architecture clé :
- **Endpoint léger `GET /api/plaquette/{year}/summary`** (`backend/routers/plaquette.py`, déclaré entre `/{year}/exists` et `/{year}` — ordre FastAPI strict) → délègue à `plaquette_service.get_summary(year)` qui lit le JSON via `_load_year` + `_backfill_p1_fields` **sans appeler** `_refresh_item_neuronx` ni `evaluate_all_items`. Retourne `{year, exists, status, has_plaquette_upload, n_items_total, n_a_challenger, n_en_discussion, n_resolu, n_risque_critique, n_risque_eleve, risque_score_global, declared_at, declaration_ref}` ou `{year, exists: false}` (200 OK jamais 404 — sidebar interroge gracieusement). Header `Cache-Control: private, max-age=10`.
- **Hook `usePlaquetteSummary(year)`** (`frontend/src/hooks/usePlaquetteCheck.ts`) — `staleTime: 30_000` + `refetchOnWindowFocus: true` + queryKey `['plaquette-summary', year]`. **Invalidation prefix-match** câblée sur 10 mutations clé (items / statut / risque) → badge se met à jour immédiatement.
- **Composant `PlaquetteSidebarBadge`** (`frontend/src/components/layout/PlaquetteSidebarBadge.tsx`) — tableau de priorités (premier match gagne) : `declare` → emerald + `Lock` / `n_risque_critique>0` → rouge `N!` / `n_risque_eleve>0` → orange `N!` / `n_a_challenger>0` → ambre `N` / sinon → `return null`. Style aligné sur les autres badges sidebar (`min-w-[18px] h-[18px] rounded-full px-1.5 text-[10px] font-bold`).
- **Card info ambre dans ComparatifTab** quand `!checkData?.ged_doc_id` — affichée en premier, message pédagogique « Aucune plaquette comptable {year} n'est encore téléversée. Vous pouvez quand même préparer la vérification… » + bouton « Téléverser la plaquette » qui `closeDrawer()` puis `navigate('/ged?type=plaquette_comptable&year={year}')`.

Le **dual entry-point** est préservé : la card « Ouvrir vérification » dans `GedDocumentDrawer` (Session 38) reste fonctionnelle et identique pour les utilisateurs qui partent du PDF physique. Le `gedDocumentId` stocké dans le store distingue les 2 modes d'ouverture mais la condition d'affichage de la card pré-réception se base sur `checkData.ged_doc_id` (= y a-t-il un PDF lié dans le JSON métier ?) pour traiter correctement le cas où l'ouverture sidebar suit un upload déjà fait par ailleurs.

**Session 40 P1 — Module Position de repli (négociation calibrée)** : ajoute un calque de négociation au-dessus des items `a_challenger`. Plutôt que challenger 100 % de chaque écart, l'utilisateur calibre une position item par item avec **% à céder**, **ton** (ferme/équilibré/conciliant) et **argumentation rédigée** auto-générée par templates BOI/CGI. Module composé sur l'évaluation risque Session 39 P2 — aucun double calcul, juste lecture des `risque_fiscal.drivers` déjà persistés.

Architecture clé :
- **Modèles** (`backend/models/plaquette_check.py`) : `ConcessionEvaluation` (pct_maintenu 0..100, montant_maintenu, montant_concede, source `auto|manual`, tone, force_score 0..1, argumentation, auto_argumentation, drivers_used) ajouté à `PlaquetteItem.concession`. `NegociationSynthesis` pour vue agrégée. `ConcessionOverridePayload` pour PATCH partiel.
- **Service `plaquette_negociation_service.py`** (~720 LOC, module pur) : algo `compute_force_score(item)` (base 0.5 + risque inversé `faible +0.25 / modere 0 / eleve -0.20 / critique -0.35` + boi_cgi_cite +0.20 + pieces_disponibles +0.15 + statut_resolu/refus +0.10 + forfait +0.10 − montant_eleve_sensible 0.10 − taux_justif_bas 0.05, clamped). Mapping `recommend_pct_maintenu` : `>= 0.80 → 100% / >= 0.60 → 75% / >= 0.40 → 50% / >= 0.20 → 25% / else 0`. Mapping `infer_tone` : `>= 0.65 ferme / >= 0.35 equilibre / else conciliant`. **39 templates** d'argumentation par catégorie × ton (forfait_repas / immobilisations / honoraires_retrocedes / quote_part_vehicule / csg_crds_split / urssaf_split / dotations_amort / blanchissage / pieces_justificatives / cadeaux_reception / telephone_internet / energie_abonnements / _fallback). Heuristique `_infer_category_key(item, mapping_flags)` qui priorise les flags du mapping puis tokens sur `categories_neuronx` + `compte_label`. `_extract_boi_ref` qui détecte BOI/CGI cité dans commentaire (regex partagée avec `plaquette_risque_service._BOI_CGI_REGEX`) ou fallback via `_DRIVER_TO_REF` mapping de `plaquette_report_service`. `evaluate_all_concessions(check, force_recompute=False)` itère items `a_challenger`, skip `source=manual` + cache `last_updated_at >= last_modified_at`, cleanup `concession=null` si statut sort de `a_challenger`. `override_item_concession` régénère l'argumentation auto si pct/tone change sans `argumentation` explicite. `compute_synthesis(check)` agrège (BNC initial via `bnc_service.compute_bnc(year)` + IR projeté via 2 appels `fiscal_service.simulate_multi`).
- **Auto-recompute** branché dans `plaquette_service.get_or_create` + `get` après `evaluate_all_items` (risque). Skip si DECLARE. Import local pour éviter cycles.
- **Enrichissement email** : `generate_challenge_email(year, nom, include_concessions=True)` bascule sur `_build_email_with_concessions` quand au moins un item a une `concession` → corps mail en 4 sections (SYNTHÈSE / ITEMS MAINTENUS / EN DISCUSSION / CONCÉDÉS) avec calcul TMI effectif `|economie_ir| / concession_totale × 100`. Sinon flow legacy challenge 100 %.
- **5 endpoints négociation** sous `/api/plaquette` : `POST /negociation/compute`, `PATCH /items/{id}/concession`, `DELETE /items/{id}/concession/override`, `POST /items/{id}/concession/regenerate-argumentation`, `GET /negociation/synthesis`. Garde `_guard_not_declared` (HTTP 423) sur les 4 mutations. Endpoint `POST /reconciliation-pdf` séparé pour générer un PDF compact orienté négociation (distinct du rapport de vérification).
- **Service `plaquette_reconciliation_pdf_service.py`** (~400 LOC) : génère un PDF paysage A4 ~27 Ko avec bandeau 6 KPIs + tableau par catégorie NeuronX (tri par ampleur d'ajustement, multi-mapping signalé) + tableau exhaustif par poste PCG (28 lignes, colonnes Plaquette / NeuronX / % / Contre-proposition / **Ajustement vs comptable** / Statut). Auto-replace en GED (template_id `plaquette_reconciliation`).

Frontend :
- **5 hooks TanStack Query** dans `usePlaquetteCheck.ts` (`useNegociationSynthesis`, `useComputeNegociation`, `usePatchItemConcession`, `useResetItemConcession`, `useRegenerateArgumentation`) + `useGenerateReconciliationPdf` — invalidations cascade `['plaquette-check']` + `['plaquette-negociation-synthesis']` + GED pour le PDF.
- **Store Zustand étendu** (`plaquetteCheckDrawerStore.ts`) : `negociationDrawerOpen: boolean`, `openNegociationDrawer`, `closeNegociationDrawer`. Pas de persist (state éphémère). `close()` du drawer parent ferme aussi le sous-drawer.
- **Sous-drawer `PlaquetteNegociationDrawer.tsx`** (~900 LOC) : 720px par défaut (resizable 550-1100 via handle drag, persisté en `localStorage['plaquette-nego-drawer-width']`), ancré à droite z-[70] avec backdrop z-[60] (au-dessus du `PlaquetteCheckDrawer` z-50). Esc handler en mode capture + stopPropagation. **Mode hybride Live/Standard** : toggle dans le header (icône `Zap`/`ZapOff`, défaut Live). Live = synthèse mise à jour côté client en temps réel pendant le drag du slider (calcul Map<item_id, pct> + recompute liveSynthesis via `useMemo`), argumentation reste différée (PATCH debounce 400ms). Cleanup auto de `livePcts` quand backend rattrape (`|backendPct - livePct| < 0.5`). **2 vues** : Cards (argumentation textarea + slider intégré, calibrage fin) et Tableau (8 colonnes compactes avec colonne **Contre-proposition** primary + colonne **Δ BNC** signée orange/vert). Tri figé par `force_score` desc à l'ouverture (deps `[orderSignature]` basée sur `item_id` seul — pas de re-sort live).
- **Card "Position de repli (optionnel)"** insérée dans `EmailTab` de `PlaquetteCheckDrawer.tsx` sous la card "Workflow recommandé" — bouton "Préparer la position de repli" (déclenche `compute` puis ouvre drawer) ou (si déjà configuré) badge "X items configurés · BNC simulé Y €" + boutons Ajuster/Réinitialiser. Bouton "Exporter PDF réconciliation" dans le footer du sous-drawer.
- **Drawers redimensionnables** : `PlaquetteCheckDrawer` (800-95vw, défaut 1100px, `localStorage['plaquette-drawer-width']`) et `PlaquetteNegociationDrawer` (550-1100px, défaut 720px) avec handle drag bord gauche (1.5px, hover `bg-primary/30`, active `bg-primary/50`, pill indicator centré) — pattern miroir `GedDocumentDrawer`.
