...
## Exclusions délibérées (GED & intégrité)

**Principe** : `sandbox/` est une file d'attente de travail, **pas** un dossier de référence. Les fichiers y sont transitoires, sans metadata OCR, sans fournisseur/date/montant validés. Les inclure dans la GED polluerait les filtres, les stats, les vues par fournisseur/catégorie avec des items vides.

**Règles à implémenter strictement :**

1. **GED ne scanne PAS `sandbox/`**
   - `ged_service.rebuild_index()` et tous les scans GED itèrent uniquement sur `en_attente/` + `traites/` (comportement actuel, juste à ne pas étendre)
   - `ged_service.get_all_documents()`, `get_ged_tree()`, `get_ged_stats()` n'incluent aucun fichier sandbox
   - Les thumbnails sandbox (`/api/sandbox/{filename}/thumbnail`) vivent dans un cache séparé (`data/sandbox_thumbs/` ou ad-hoc) et ne sont **jamais** écrits dans `data/ged/thumbnails/`

2. **Intégrité des liens — sandbox exclu**
   - `get_all_referenced_justificatifs()` : aucune opération ne peut référencer un fichier sandbox (par construction, l'association n'est possible qu'après OCR). Ne pas scanner sandbox/ pour cette fonction.
   - `scan_link_issues()` : ne considère ni les fichiers sandbox/ comme source possible, ni ne génère d'alertes sur eux. Les 6 catégories d'incohérences existantes (duplicates, misplaced, orphans, ghost_refs, hash_conflicts) restent scopées à `en_attente/` + `traites/`
   - `repair_links()` : inchangé, ne touche jamais sandbox/

3. **Résolution cross-location — sandbox isolé**
   - `justificatif_service.get_justificatif_path()` continue de résoudre `en_attente/` puis `traites/` uniquement. **Ne pas ajouter sandbox/ à la chaîne.**
   - Les fichiers sandbox ont leur propre résolveur dédié : `sandbox_service.get_sandbox_path()` (nouveau, simple)
   - Les endpoints `/api/justificatifs/...` ne retournent jamais un fichier sandbox. Les endpoints `/api/sandbox/...` ne retournent jamais un fichier en_attente/traites.

4. **Stats enrichies pour visibilité**
   - `GET /api/justificatifs/stats` enrichi : `{en_attente: N, traites: N, sandbox: N, total: N+N+N}`
   - Le front badge sidebar OCR lit `sandbox` uniquement (pas besoin d'un endpoint séparé)
   - La page Pipeline home (`/`) peut afficher un mini-indicateur « N fichiers en Sandbox » à côté du badge % global si > 0

5. **Safety net reporté**
   - Pas d'alerte « fichier sandbox > 7 jours » dans AlertesPage pour V1. On verra en usage si l'oubli est un vrai problème.
   - Noter dans CLAUDE.md "On the horizon" comme évolution possible.
...
6. `backend/services/sandbox_service.py` — split `_process_from_sandbox`, watchdog conditionnel, in-memory arrivals, rename + process APIs, nouvelle `get_sandbox_path()`, `count_sandbox_files()`
...
11. `backend/services/justificatif_service.py` — **vérifier** que `get_justificatif_path()`, `get_all_referenced_justificatifs()` et `scan_link_issues()` **n'incluent pas** sandbox/ (ajouter un commentaire explicite dans chaque fonction pour prévenir les ajouts futurs). Enrichir `get_stats()` avec la clé `sandbox` (via `sandbox_service.count_sandbox_files()`).
12. `backend/services/ged_service.py` — **vérifier** que `rebuild_index()`, `get_all_documents()`, `get_ged_tree()`, `get_ged_stats()` ignorent sandbox/. Pas d'extension des dossiers scannés. Commentaire explicite.
13. `frontend/src/components/ocr/OcrPage.tsx` — onglet Sandbox en 1er
14. `frontend/src/components/ocr/FilenameEditor.tsx` — prop optionnel `endpoint` pour router rename sandbox vs justificatif, OU créer un hook wrapper
15. `frontend/src/components/layout/Sidebar.tsx` — badge compteur OCR alimenté par `stats.sandbox`
16. `frontend/src/components/settings/...` — section Sandbox
17. `frontend/src/hooks/useSandbox.ts` — si existant, enrichir pour exposer `is_canonical` et `arrived_at` dans les items
18. `frontend/src/hooks/useJustificatifStats.ts` (ou équivalent) — type retour enrichi avec `sandbox: number`
19. `frontend/src/components/pipeline/PipelinePage.tsx` — mini-indicateur « N en Sandbox » si `stats.sandbox > 0` (optionnel, safety net visuel)
20. `api-reference.md` — documenter les 3 nouveaux endpoints + enrichissement `/stats`
21. `architecture.md` — schéma de flux mis à jour (sandbox = inbox) **+ section « Exclusions GED »** qui acte que sandbox/ est hors périmètre GED et intégrité
22. `CLAUDE.md` — section OCR mise à jour : « Sandbox » comme premier onglet, watchdog conditionnel, mode manuel par défaut, **règle d'exclusion GED**
23. `CHANGELOG.md` — entry Session X
...
- [ ] **Exclusion GED** : un fichier dans sandbox/ **n'apparaît pas** dans la page GED (`/ged`), ni dans les filtres par fournisseur/catégorie/année
- [ ] **Stats enrichies** : `GET /api/justificatifs/stats` retourne bien `{en_attente, traites, sandbox, total}` avec `total = en_attente + traites + sandbox`
- [ ] **Intégrité links** : `GET /api/justificatifs/scan-links` (dry-run) ne remonte jamais un fichier sandbox dans les 6 catégories d'incohérences
- [ ] **Sidebar badge** : le compteur sur l'entrée OCR reflète bien `stats.sandbox` (croît de 1 au dépôt, décrôit de 1 au `[Lancer OCR]` ou `[Supprimer]`)
...
- **GED : ne PAS étendre le périmètre à sandbox/** — c'est un choix d'architecture délibéré (voir section « Exclusions délibérées »). Toute tentative d'ajouter sandbox/ aux scans GED doit être rejetée en review.
- **Alertes sandbox > N jours** : pas dans V1. Noter dans CLAUDE.md "On the horizon" comme évolution possible si l'usage montre un besoin.
