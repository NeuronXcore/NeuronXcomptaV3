...
**`run_auto()` — rapprochement automatique :**
- Pour chaque opération ventilée, itérer sur ses sous-lignes individuellement (pas l'op parente)
- Le scoring utilise `sous_ligne.montant` au lieu de `op.debit` ou `op.credit`
- La date reste celle de l'op parente (c'est le mouvement bancaire)
- L'association écrit dans `ventilation[line_index].justificatif` (pas `op.justificatif`)
- Une op ventilée avec N sous-lignes peut matcher N justificatifs différents dans le même run
- Les opérations non ventilées gardent le comportement actuel inchangé
- Ne pas tenter de matcher l'op parente elle-même (son montant total ne correspond à aucun justificatif)

**`get_suggestions()` / `batch-hints` / `batch-justificatif-scores` :**
- Ajouter un paramètre optionnel `ventilation_index: Optional[int]`
- Si fourni → scorer avec `sous_ligne.montant`, retourner les suggestions pour cette sous-ligne
- Si non fourni et op ventilée → retourner un flag `ventilated: true` avec la liste des sous-lignes (le frontend affiche le sélecteur)
- Si non fourni et op non ventilée → comportement actuel

**`associate_manual()` :**
- Accepter `ventilation_index: Optional[int]`
- Si fourni, écrire dans `ventilation[ventilation_index].justificatif`
- Sinon, écrire dans `op.justificatif` (comportement actuel)

**`get_unmatched()` :**
- Compter les sous-lignes sans justificatif (pas l'op parente) dans le total opérations non rapprochées
