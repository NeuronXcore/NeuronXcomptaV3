...
| `/reports` | ReportsPage | **Rapports V2** : 2 onglets (Générer avec checkboxes modernes catégories + "Tout sélectionner" + bouton batch 12 mois, Bibliothèque avec arbre par date/catégorie), checkboxes toggle modernes sur les cartes, sélection multi-rapports + bouton "Exporter pour le comptable" (ZIP), suppression individuelle/totale avec toast confirmation centré, ouverture native (Aperçu/Numbers/Excel), favoris épinglés, comparaison side-by-side, formats PDF (logo, colonnes Justificatif ☑/☐ + Commentaire, EUR)/CSV (`;` FR, 8 colonnes)/Excel, déduplication, index JSON, preview drawer 800px avec édition titre/description |
