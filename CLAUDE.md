...
- **OCR**: EasyOCR with pdf2image, cache `.ocr.json` alongside PDFs. OCR page is the primary entry point for justificatif uploads (batch upload PDF/JPG/PNG + immediate OCR). **Auto-rename post-OCR** : après extraction, les justificatifs sont renommés automatiquement en `fournisseur_YYYYMMDD_montant,XX.pdf` via `naming_service.py` (`normalize_supplier()`, `build_convention_filename()`, `deduplicate_filename()`). Renommage met à jour PDF + .ocr.json + associations opérations + GED metadata. Convention virgule décimale. Idempotent (ne re-renomme pas les fichiers déjà conformes). **FilenameEditor** : composant frontend inline-editable dans l'historique OCR et le drawer justificatif, avec suggestion convention (bouton Sparkles) et badge "auto" si renommé automatiquement. **Aperçu PDF hover** : icône œil dans l'historique OCR → popover 300×400px au survol. Historique trié par `processed_at` (date de traitement OCR, plus récent en premier).
- **Sandbox Watchdog**: `watchdog` library monitors `data/justificatifs/sandbox/` for PDF/JPG/PNG, auto-converts images to PDF, auto-OCR + auto-rename + SSE notifications (enrichies `auto_renamed` + `original_filename`)
...
│   └── services/               # Business logic (23 services, incl. ged_service.py, amortissement_service.py, fiscal_service.py, template_service.py, previsionnel_service.py, task_service.py, ml_monitoring_service.py, email_service.py, email_history_service.py, naming_service.py)
├── frontend/
│   └── src/
...
| justificatifs | `/api/justificatifs` | GET /, GET /stats, POST /upload, GET /reverse-lookup/{filename}, POST /associate, POST /dissociate, POST /{filename}/rename |
...
| `/justificatifs` | JustificatifsPage | **Vue opérations-centrée** : sélecteur année/mois (store Zustand), tableau triable 7 colonnes avec **checkboxes multi-sélection** (ops sans justif), filtre sans/avec justif (défaut: sans), 4 MetricCards, **catégorie/sous-catégorie éditables inline** (select dropdowns avec sauvegarde auto), **bandeau CTA "Associer automatiquement"** (ambre, visible quand ops sans justif, toast cliquable pour association manuelle), **barre flottante batch** (sélection → "Reconstituer (N)" → `BatchReconstituerDrawer` avec choix template par groupe), `JustificatifAttributionDrawer` (800px split resizable, suggestions scorées + **recherche libre** dans tous les justificatifs en attente avec debounce, hover preview PDF, bouton Attribuer orange, score affiché en %), flash highlight, sandbox SSE |
