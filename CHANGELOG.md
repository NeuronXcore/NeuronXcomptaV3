...
**Backend** :
- **Enum `RisqueNiveau`** + `RisqueDriver` + `RisqueFiscalEvaluation` + `RisqueOverrideRequest` ([`backend/models/plaquette_check.py`](backend/models/plaquette_check.py)). Extensions : `PlaquetteItem.risque_fiscal: Optional[RisqueFiscalEvaluation]` + `PlaquetteCheck.risque_score_global: Optional[float]`.
- **Service risque** [`backend/services/plaquette_risque_service.py`](backend/services/plaquette_risque_service.py) (**NOUVEAU**, ~400 LOC, module pur) : constantes `CATEGORIES_SENSIBLES` (whitelist 9 catégories), seuils `TAUX_JUSTIF_BAS=0.80` / `ECART_N1_ANORMAL=0.50` / `MONTANT_ELEVE_SEUIL=5000` / `SCORE_THRESHOLDS` (0/2/3/999 — CRITIQUE réservé aux overrides manuels). 5 drivers aggravants (+1) + 3 atténuants (-1). Helpers `evaluate_item` / `evaluate_all_items` (cache implicite via `last_evaluated_at >= last_modified_at`, skip si `status==DECLARE`) / `override_item_risque` / `reset_item_risque_auto` / `compute_global_score` / `get_top_risques`. Internal `_compute_taux_justif_categorie(year)` itère ops + ventilations sub_lines.
- **Intégration auto** : `plaquette_service.get_or_create` + `get` appellent `evaluate_all_items(data)` à chaque GET. Préserve les overrides manuels.
- **4 endpoints** sous `/api/plaquette` :
  - `POST /{year}/risque/recompute` — force recalc tous items (423 si DECLARE)
  - `PATCH /{year}/items/{item_id}/risque` body `RisqueOverrideRequest` — override manuel, motif obligatoire (423 si DECLARE)
  - `DELETE /{year}/items/{item_id}/risque/override` — reset auto + recompute immédiat (423 si DECLARE)
  - `GET /{year}/risque/top?limit=5` — autorisé en DECLARE (lecture seule)
- **PDF section 7** dans [`plaquette_report_service.py`](backend/services/plaquette_report_service.py) : helpers `_get_top_risques_from_data`, `_refs_from_top_risques` (auto-inject `art_L169_LPF` + `art_L102_B_LPF` toujours + refs déclenchées par drivers via map `_DRIVER_TO_REF`), `_format_score_global`, `_render_risque_section`, `_render_risque_card` (header coloré par niveau `#dc2626`/`#ea580c`/`#eab308`/`#16a34a`, drivers ± colorés, motif override en italique). Section insérée AVANT l'annexe juridique (renumérotée 8).
- **Annexe juridique enrichie** : 4 nouvelles entrées `_BOI_CGI_REFERENCES` — `art_L169_LPF` (prescription droit commun 3 ans), `art_L102_B_LPF` (conservation pièces 6 ans), `art_240_CGI_DAS2` (DAS-2 honoraires rétrocédés 1 200 €), `art_39_4_CGI` (plafonds amortissement véhicule classe CO2). Dédup ordre + injection auto.

**Frontend** :
- **Types** dans `types/index.ts` : `RisqueNiveau` / `RisqueDriver` / `RisqueFiscalEvaluation` + extensions + payloads `RisqueOverridePayload` / `TopRisquesResult` / `RecomputeRisqueResult`.
- **4 hooks** ([`frontend/src/hooks/usePlaquetteCheck.ts`](frontend/src/hooks/usePlaquetteCheck.ts)) : `useRecomputeRisque`, `usePatchItemRisque`, `useResetItemRisque`, `useTopRisques(year, limit=5)`. Invalidations cascade `['plaquette-check']` + `['plaquette-top-risques']`.
- **3 nouveaux composants** dans `frontend/src/components/plaquette/` :
  - **`PlaquetteRisqueChip.tsx`** — pastille compacte 22px par niveau (4 couleurs + icônes `AlertOctagon`/`AlertTriangle`/`AlertCircle`/`ShieldCheck`) + badge `M` purple si override manuel + tooltip riche au hover (drivers ± colorés, pièces, motif, CTA modifier). Prop `compact` pour vue table.
  - **`PlaquetteRisqueOverrideModal.tsx`** (540px z-[70], mount conditionnel) — édition manuelle : calcul auto affiché en référence, 4 radio niveaux avec descriptions + ring sélection, textarea motif obligatoire, bouton secondaire « Repasser en automatique ».
  - **`PlaquetteRisqueDrawerSummary.tsx`** — affiché sous le bandeau read-only de l'onglet Comparatif : score global gros + niveau dominant coloré + 4 mini-compteurs + bouton « Recalculer ».
- **Modifications `PlaquetteCheckDrawer`** : states `riskOverrideItemId` + `riskFilter`. ComparatifTab reçoit `checkData` + `riskFilter` + `onRiskFilterChange` + `onOpenRiskOverride`. Summary inséré sous le bandeau read-only. Chips filtre 6 options (Tous / 4 niveaux / M manuel) avec compteur. Nouvelle colonne `Risque` entre Statut et Commentaire (10 cols au total). EmailTab : bandeau alerte orange si `nbRisqueEleveOuCritique > 0` (items eleve/critique non resolu/refus_justifie).

**Smoke test workflow complet** : ouvrir plaquette 2025 → vérifier les risques calculés (1 ELEVE véhicule + 6 MODERE + 21 FAIBLE) → score global 0.39 → override 1 item en critique avec motif « test » → badge `M` apparaît + chip rouge → reset override → repasse en auto → recalc global force-recompute → générer PDF rapport : section 7 présente avec top 5, annexe juridique enrichie de L169/L102 B/240/39-4. En DECLARE : tous endpoints risque mutation refusés 423, GET top reste autorisé en lecture seule.

**Limitations connues** :
- Le score CRITIQUE auto plafonne à 4 (5 aggravants − 1 atténuant) → ELEVE max en auto. CRITIQUE est intentionnellement réservé aux overrides manuels (seuil 999 dans `SCORE_THRESHOLDS`).
- Le calcul `taux_justif_categorie` itère toutes les ops + ventilations à chaque `evaluate_all_items` non-cached — coût O(N ops) à chaque GET. Pour des gros volumes (> 10 000 ops/an), envisager un cache mtime sur le fichier d'opérations.
- L'override `auto_calcule=False` ne recalcule plus les drivers : si l'utilisateur modifie l'item plus tard (commentaire avec ref BOI par exemple), il faut explicitement « Repasser en automatique » pour bénéficier de la nouvelle évaluation.

---

### Added (2026-05-18) — Plaquette : cycle de vie + snapshot final + attachements journal (Session 39 — P1)

Extension du module Plaquette (Session 38) pour couvrir le cycle complet jusqu'à la déclaration 2042 : 3 statuts de cycle de vie, freeze progressif des items, snapshot immuable au moment du `declare`, attachements PDF/EML par entrée journal, vue alternative « par item ». Le snapshot final reste consultable durant toute la prescription fiscale (4 ans, art. L169 LPF).

**Backend** :
- **Enum `PlaquetteCheckStatus`** ([`backend/models/plaquette_check.py`](backend/models/plaquette_check.py)) : `en_cours` / `validation_finale` / `declare`. Matrice de transitions stricte : `en_cours ↔ validation_finale` libre, `validation_finale → declare` UNIQUEMENT via endpoint `/finalize` (irréversible).
- **Modèle `JournalAttachment`** : `filename`, `storage_path`, `size_bytes`, `mime_type`, `uploaded_at`. Extension `JournalEntry.attachments: list[JournalAttachment]` + 5 nouveaux champs sur `PlaquetteCheck` (`status`, `validated_at`, `declared_at`, `declaration_ref`, `final_snapshot_ged_doc_id`). Migration douce — Pydantic injecte `status=en_cours` par défaut sur les JSON pré-P1.
- **Service finalisation** [`backend/services/plaquette_finalization_service.py`](backend/services/plaquette_finalization_service.py) (**NOUVEAU**, module pur) : `transition_status(year, new_status, declaration_ref?)` (refuse `→DECLARE`), `is_item_modifiable`, `is_journal_appendable` (toujours True — questions fisc post-déclaration), `is_status_revertable`, et `finalize(year, declaration_ref, declared_at?)` qui chaîne atomique : (1) vérifie `status==validation_finale`, (2) génère PDF watermarké via `plaquette_report_service.generate_and_register(year, final=True)`, (3) copie `data/plaquette_check/{year}.json` → `data/plaquette_check/{year}/final_snapshot.json` (atomique tempfile + os.replace), (4) PDF enregistré en GED avec `protected=True` + `template_id="plaquette_check_final"`, (5) update `PlaquetteCheck.status=declare` + horodatage + ref + doc_id snapshot.
- **GED — flag `protected`** ([`backend/services/ged_service.py`](backend/services/ged_service.py)) : nouveau param `protected: bool = False` sur `register_rapport()` + champ top-level `"protected"` sur le doc dict. `delete_document()` retourne `False` sans rien faire si `doc.get("protected") is True` (log warning) — applique à toutes les sources de suppression (API GED, `_delete_previous_reports` plaquette qui skip aussi `report_type=plaquette_check_final` par belt-and-suspenders). Rétrocompat : docs pré-P1 sans champ se comportent comme `False`.
- **Watermark PDF** ([`backend/services/plaquette_report_service.py`](backend/services/plaquette_report_service.py)) : param `final: bool = False` propagé à `generate_report()` → `_render_pdf()` → callback `_on_page` qui combine `_footer` + nouveau helper `_draw_watermark(canvas_obj, doc, declared_at)`. Watermark = `Helvetica-Bold 48` rotation 45° au centre de chaque page avec `setFillColorRGB(0.85, 0.85, 0.9, alpha=0.18)` + fallback `setFillAlpha`. Texte « VERSION DÉFINITIVE » + sous-titre 14pt « Déclarée le JJ/MM/AAAA ». Filename préfixé `plaquette_check_final_{year}_{ts}.pdf`.
- **Attachements journal** ([`backend/services/plaquette_service.py`](backend/services/plaquette_service.py)) : 4 fonctions publiques `add_journal_attachment` / `remove_journal_attachment` / `get_journal_attachment_path` / `get_journal_grouped_by_item` + constantes `ALLOWED_ATTACHMENT_MIMES` (pdf, png, jpeg, webp, eml, zip) + `MAX_ATTACHMENT_SIZE = 10 Mo`. Stockage `data/plaquette_check/{year}/journal_attachments/{slug}_{token8}.{ext}` (anti-collision via `secrets.token_hex(4)`). Validation mime + taille → `ValueError(code)` traduits par le router en 413/415/404/400.
- **Garde modifiabilité** : wrapper `_guard_modifiable(data)` lève `PermissionError("plaquette frozen status=...")` si `!is_item_modifiable(data)`. Appelé dans `patch_item`, `add_item`, `delete_item`, `patch_totaux`, `set_ged_ref` (PAS dans `add_journal_entry` ni les fonctions d'attachements — journal toujours appendable, même post-DECLARE).
- **6 nouveaux endpoints** sous `/api/plaquette` : `PATCH /{year}/status`, `POST /{year}/finalize`, `POST /{year}/journal/{entry_id}/attachments` (multipart `file`), `GET /{year}/journal/{entry_id}/attachments/{filename}` (FileResponse inline avec mime du JSON), `DELETE /{year}/journal/{entry_id}/attachments/{filename}`, `GET /{year}/journal/grouped-by-item`. Route statique `grouped-by-item` déclarée AVANT routes dynamiques (pattern strict FastAPI). **Try/except `PermissionError → 423 Locked`** ajouté à 5 endpoints mutation existants. Codes : 400 (transition invalide / ref vide), 404 (année / entry_id), 413 (> 10 Mo), 415 (mime), 423 (gelé).

**Frontend** :
- **Types** ([`frontend/src/types/index.ts`](frontend/src/types/index.ts)) : `PlaquetteCheckStatus`, `JournalAttachment`, extensions `PlaquetteJournalEntry.attachments[]` + `PlaquetteCheck` (5 champs cycle de vie) + payloads `PlaquetteStatusUpdatePayload` / `FinalizePlaquettePayload` / `FinalizePlaquetteResult` / `JournalGroupedByItem` (convention projet — pas de fichier séparé `types/plaquette.ts`).
- **Hooks** ([`frontend/src/hooks/usePlaquetteCheck.ts`](frontend/src/hooks/usePlaquetteCheck.ts)) : `usePatchPlaquetteStatus`, `useFinalizePlaquette` (invalidations cascade `['plaquette-check']` + GED + `['plaquette-reports']` + `['livret']`), `useUploadJournalAttachment` (FormData direct via fetch — `api.post` force JSON), `useDeleteJournalAttachment`, `useJournalGroupedByItem` + helper URL `journalAttachmentUrl(year, entryId, filename)` + helpers `isPlaquetteEditable` / `isPlaquetteStatusRevertable`.
- **Store** ([`frontend/src/stores/plaquetteCheckDrawerStore.ts`](frontend/src/stores/plaquetteCheckDrawerStore.ts)) : ajout `journalView: 'timeline' | 'by-item'` + `setJournalView`, défaut `'timeline'`.
- **5 nouveaux composants** dans `frontend/src/components/plaquette/` :
  - **`PlaquetteStatusBadge.tsx`** — pill colorée par statut avec icône `FileEdit` ambre / `CheckCircle2` sky / `Lock` emerald + tooltip declared_at + declaration_ref.
  - **`PlaquetteStatusActionsMenu.tsx`** — boutons contextuels : en_cours → primary « Passer en validation finale » / validation_finale → outline « Revenir en cours » + confirmation inline + primary emerald « Finaliser et déclarer » / declare → disabled avec tooltip prescription.
  - **`PlaquetteFinalizationModal.tsx`** — wizard 3 étapes 640px z-[70] backdrop z-[60] : récap pré-vol avec compteurs + warning si non résolus / saisie `declaration_ref` + date picker + preview filename / confirmation + bouton emerald `Lock` → `useFinalizePlaquette` + toast 6s.
  - **`JournalAttachmentList.tsx`** — chips horizontales (icône selon mime + filename tronqué + taille + Trash2 au hover) + drop zone react-dropzone avec validation client 10 Mo + accept mime + toast erreur 413/415.
  - **`JournalByItemView.tsx`** — items expandables (consomme `useJournalGroupedByItem`) avec entries chronologiques desc, toggle « Voir aussi les items sans échange » avec compteur, intègre `JournalAttachmentList`.
- **PlaquetteCheckDrawer modifié** : header ajoute badge + actions menu à droite du titre (`flex-wrap`), wizard rendu après le drawer, onglet Comparatif reçoit prop `readOnly` propagé aux 4 inputs `Row` + bandeau bleu sky / vert emerald en haut selon statut, onglet Journal segmented control `Timeline | Par item`, tab Email Challenge bannière info verte + boutons regen/finalisation désactivés si `status==declare`.

**Smoke test workflow complet** : ouvrir plaquette 2025 (status `en_cours`) → passer en `validation_finale` (items deviennent read-only, mutations 423) → upload attachement journal OK même verrouillé → cliquer « Finaliser et déclarer » → wizard 3 étapes → saisir `declaration_ref` → confirmer → PDF watermarké en GED `protected=true` + JSON figé dans `{year}/final_snapshot.json` + status `declare` → tentative `delete_document` sur le snapshot refusée silencieusement → journal toujours appendable post-DECLARE.

**Mauvais usage à éviter** : `register_rapport(protected=True)` est exclusivement pour les snapshots de déclaration (plaquette finalisée, futurs snapshots livret cloture). Ne JAMAIS l'appliquer aux rapports standards (Charges forfaitaires, Amortissements, BNC annuel, etc.) — ils ont besoin de pouvoir être supprimés et régénérés librement.
